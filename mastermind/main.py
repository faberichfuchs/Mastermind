...
DARK_GREY = (96, 96, 96)
SILVER = (192, 192, 192)
colors = {'RED': (255, 0, 0), 'GREEN': (0, 255, 0), 'BLUE': (0, 0, 255), 'CYAN': (0, 255, 255), 'MAGENTA': (255, 0, 255), 'YELLOW': (255, 255, 0)}
...
for i in range(0, 11):
    ...
    hints.append(temp.copy())
    temp.clear()
# 6 colors that can be selected
selectableColors.update(
    {'RED': pygame.draw.circle(screen, RED, (int(screenWidth / 6) * 1 - 18, screenHeight - 55), 15, 0)})
selectableColors.update(
    {'GREEN': pygame.draw.circle(screen, GREEN, (int(screenWidth / 6) * 2 - 18, screenHeight - 55), 15, 0)})
selectableColors.update(
    {'BLUE': pygame.draw.circle(screen, BLUE, (int(screenWidth / 6) * 3 - 18, screenHeight - 55), 15, 0)})
selectableColors.update(
    {'CYAN': pygame.draw.circle(screen, CYAN, (int(screenWidth / 6) * 4 - 18, screenHeight - 55), 15, 0)})
selectableColors.update(
    {'MAGENTA': pygame.draw.circle(screen, MAGENTA, (int(screenWidth / 6) * 5 - 18, screenHeight - 55), 15, 0)})
selectableColors.update(
    {'YELLOW': pygame.draw.circle(screen, YELLOW, (int(screenWidth / 6) * 6 - 18, screenHeight - 55), 15, 0)})
# confirm button
confirm = pygame.draw.rect(screen, WHITE, [10, screenHeight - 35, int(screenWidth / 2) - 15, 30], 0)
# solve button
solve = pygame.draw.rect(screen, WHITE,
                         [int(screenWidth / 2) + 5, screenHeight - 35, int(screenWidth / 2) - 10, 30], 0)
...
while carryOn:
    ...
    for event in pygame.event.get():  # User did something
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            # Color selection
            for c in colors:
                for sc in selectableColors:
                    if selectableColors[c].collidepoint(pos):
                        selected_color = c
